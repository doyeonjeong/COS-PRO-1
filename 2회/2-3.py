...
def solution(num):
    ...
    while True:
        next_num += 1
        length = func_b(next_num)  # next_num의 자릿수 구하기
        if length % 2:  # 홀수라면 next_num += 1
            continue
        divisor = func_a(length // 2)  # 6자리라면 3을 넘겨주어 1000으로 반환한다.
        front = next_num // divisor    # 그럼 앞 3자리
        back = next_num % divisor      # 뒤 3자리로 나눌 수 있다.
